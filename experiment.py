...
class LSTMLanguageClassifier(nn.Module):
    ...
    def __init__(self, vocab_size,embedding_dim, hidden_dim,hidden_dim2, label_size,batch_size,use_gpu):
        ...
        self.lstm_to_hidden = nn.Linear(hidden_dim,hidden_dim2,bias=True)
        self.hidden_to_output=nn.Linear(hidden_dim2,label_size,bias=True)
    ...
    def forward(self, input):
        ...
        h = self.lstm_to_hidden(lstm_out[-1])
...
def run_epoch(data,model,letter_to_index,loss_function,optimizer,train):
    ...
    total = 0.0
    for i, o in data:
        input_tensor = torch.zeros((len(i), 1)).long()
        for n, l in enumerate(i):
            input_tensor[n] = letter_to_index[l]
...
def main():
    ...
    letter_to_index,index_to_letter=letter_index_structures()
    train_set,test_set= create_data_set("pos_examples","neg_examples")
    model=LSTMLanguageClassifier(VOCAB_SIZE,EMBEDDING_DIM,HIDDEN_DIM,HIDDEN_DIM2,LABEL_SIZE,1,use_gpu)
    if use_gpu:
        model= model.cuda()
